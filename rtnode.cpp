...
RTNode::RTNode(RTree *rt){//create a new RTNode on disk. using for creating RTree
    char *b;
    int header_size;//size of (char + int)
    Entry * d;
    int i;
    ...
    capacity = (rt -> file -> get_blocklength() - header_size) / (d -> get_size());//number of entries
    delete d;
...
RTNode::RTNode(RTree *rt, int _block){//restore RTNode from disk; '_block' is a number location where the block data located
	char *b;
    int header_size;//level:char + num_entries :int
    Entry * d;
    int i;
    ...
    if (rt -> cache == NULL) // no cache
        rt -> file -> read_block(b, block);//load from disk to buffer, data located at 'block' position
    else
...
RTNode::~RTNode(){	
    ...
    if (dirty){//memory data has been changed
...
float* RTNode::get_mbr(){//get mbr of all entries of current RTNode
    int i, j;
    float *mbr;
        ...
        if (num_entries == (capacity - 1)){
            if (my_tree->re_level[0] == FALSE && my_tree -> root_ptr -> level != level){
                ...
                for (i = 0; i < dimension; i++)
                     center[i] = (float)((mbr[2*i] + mbr[2*i+1]) / 2.0);
